...
The scheme and implementation presented here is a kind of "school-book" variant of the scheme and should not be used in real world applications.
...
For $\beta$ sufficiently small (we will see below what we mean by "sufficiently small") let $[\beta] := \{-\beta, -\beta +1, ... , -1, 0, 1, ..., \beta\}$. For a polynomial $f$, we denote by $f \leftarrow [\beta]$ the fact that the coefficients of $f$ have been chosen uniformly at random from $[\beta]$.
...
To instantiate the encryption scheme, we choose two integers $q$ and $p$ relatively prime, where $q$ is larger than $p$. Typically, $q$ is a power of $2$, $p$ is set to $3$, furthermore $p = 2\cdot \beta +1$, i.e., for $p=3$ we have $[\beta ] = \{-1, 0, 1\}$.
...
- for given $(a,a \cdot s +e)$ find $e$ and $s$
...
The cleartext message is encoded as a small polynomial $m$ in $\mathcal{R}_{p,x^n-1}$, i.e., the coefficients of $m$ are in $[\beta]$.

To encrypt $m$ the sender furthermore creates a random polynomial $r \in \mathcal{R}_{q,x^n-1}$, where again the coefficients of $r$ are small ($r \leftarrow  [\beta]$).
...
Note that multiplication is done in $\mathcal{R}_{q,x^n-1}$, thus $f$ and $f_q^{-1}$ cancel out and we are left with $p \cdot g \cdot r + f\cdot m$. We would now like to work modulo $p$, but need to be careful not to introduce failures, since we apply modulo $q$ and modulo $p$ in a certain order. In general, this may lead to problems, however, if reduction modulo $q$ does not change any parameter (coefficient), we are fine.

Looking at the polynomials and their coefficients (taken from $[\beta]$), and using the fact that we set $p=3$, thus $\beta =1$, and the fact that our polynomials have at most $2\cdot d$ non-zero coefficients, we know that the coefficients cannot exceed $3 \cdot 2\cdot d + 2 \cdot d$. As a consequence, if we require $q/2 > 8 \cdot d$ we have a guarantee that reduction modulo $q$ does not change any of the coefficients and that we are fine.
